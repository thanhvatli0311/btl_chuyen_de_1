...
<div class="container mt-4">
    <div class="row">
        <div class="col-md-8 offset-md-2">
            <h1 class="display-5 mb-4">✏️ Chỉnh Sửa Mã Giảm Giá: <strong>{{ $code->code }}</strong></h1>
            ...
            <form action="{{ route('admin.discount-codes.update', $code) }}" method="POST" class="card shadow-lg">
                @csrf
                @method('PUT')
                <div class="card-body p-4">
                    <!-- Mã Giảm Giá (không có thể sửa) -->
                    <div class="mb-3">
                        <label class="form-label">🎯 Mã Giảm Giá:</label>
                        <input type="text" class="form-control" value="{{ $code->code }}" disabled>
                        <small class="text-muted">Không thể thay đổi mã đã tạo</small>
                    </div>
                    ...
                    <div class="mb-3">
                        ...
                        <textarea class="form-control @error('description') is-invalid @enderror" 
                                  name="description" rows="2" placeholder="Mô tả ngắn về mã giảm giá">{{ old('description', $code->description) }}</textarea>
                        @error('description')
                    ...
                    <div class="mb-3">
                        ...
                        <div class="btn-group w-100" role="group">
                            <input type="radio" class="btn-check" name="type" id="percentage" value="percentage" 
                                   {{ old('type', $code->type) === 'percentage' ? 'checked' : '' }} required>
                            ...
                            <input type="radio" class="btn-check" name="type" id="fixed_amount" value="fixed_amount"
                                   {{ old('type', $code->type) === 'fixed_amount' ? 'checked' : '' }} required>
                    ...
                    <div class="mb-3">
                        ...
                        <input type="number" class="form-control @error('value') is-invalid @enderror" 
                               ...
                               step="0.01" value="{{ old('value', $code->value) }}" required>
                        @error('value')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    </div>
                    ...
                    <div class="mb-3">
                        ...
                        <input type="number" class="form-control @error('minimum_order_amount') is-invalid @enderror" 
                               ...
                               step="0.01" value="{{ old('minimum_order_amount', $code->minimum_order_amount) }}">
                    ...
                    <div class="mb-3">
                        ...
                        <input type="number" class="form-control @error('usage_limit') is-invalid @enderror" 
                               ...
                               min="1" value="{{ old('usage_limit', $code->usage_limit) }}">
                        <small class="text-muted">Số lần tối đa mã này có thể được sử dụng. Hiện tại: {{ $code->usage_count }} / {{ $code->usage_limit ?? '∞' }}</small>
                        @error('usage_limit')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    </div>
                    ...
                    <div class="mb-3">
                        ...
                        <input type="date" class="form-control @error('valid_from') is-invalid @enderror" 
                               name="valid_from" value="{{ old('valid_from', $code->valid_from->format('Y-m-d')) }}" required>
                        @error('valid_from')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    </div>
                    ...
                    <div class="mb-3">
                        ...
                        <input type="date" class="form-control @error('valid_until') is-invalid @enderror" 
                               name="valid_until" value="{{ old('valid_until', $code->valid_until?->format('Y-m-d')) }}">
                    ...
                    <div class="alert alert-info">
                        ...
                        📅 Tạo: {{ $code->created_at->format('d/m/Y H:i') }}<br>
                        📅 Cập Nhật: {{ $code->updated_at->format('d/m/Y H:i') }}<br>
                        📊 Đã Dùng: {{ $code->usage_count }} lần
                    </div>
                </div>
